...
can_ok( $class, qw(new inc use_counter counter is_remove is_add is_debug) );
dies_ok { Jenkins::i18n::ProcOpts->new( 'foo', 'bar', 1, 1, 1, 0 ) }
...
my $instance = Jenkins::i18n::ProcOpts->new( 'foo', 'bar', 1, 0, 0, 0 );
...
is_deeply(
    ...
    [
        ...
        'source_dir', 'target_dir', 'use_counter'
    ],
    'instance has the expected attributes'
);
is( $instance->counter, 0, 'got the expected files counter' );
ok( $instance->inc, 'can invoke inc' );
is( $instance->counter, 1, 'got the expected files counter' );
...
$instance = Jenkins::i18n::ProcOpts->new( 'foo', 'bar', 0, 0, 0, 0 );
my $result;
